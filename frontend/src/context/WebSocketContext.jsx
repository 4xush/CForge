...
export const WebSocketProvider = ({ children }) => {
    ...
    async (token, userId) => {
      ...
      try {
        ...
        if (connectionAttemptRef.current < maxRetries) {
          const delay =
            retryDelay * Math.pow(1.5, connectionAttemptRef.current - 1);
          ...
          toast.error(
            "Failed to connect to chat server after multiple attempts."
          );
          connectionAttemptRef.current = 0;
        }
      }
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      const isPageReloading =
        sessionStorage.getItem("pageReloading") === "true";
  ...
  const joinRoom = useCallback(
    (roomId) => {
      ...
      if (!webSocketService.isSocketConnected()) {
        console.warn(
          "Join room cancelled: WebSocket not connected. Attempting reconnect."
        );
        const token = localStorage.getItem("app-token");
        if (token) {
          connectWithRetry(token, authUser._id);
  ...
  const sendMessage = useCallback(
    (roomId, messageContent) => {
      ...
      if (!webSocketService.isSocketConnected()) {
        console.warn(
          "Send message cancelled: WebSocket not connected. Attempting reconnect."
        );
