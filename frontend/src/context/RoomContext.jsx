...
export const RoomProvider = ({ children }) => {
  ...
  const markRoomAsRead = useCallback(
    async (roomId) => {
      try {
        ...
        await ApiService.post(`/rooms/${roomId}/update-last-seen`);
      } catch (error) {
        console.error("Failed to mark room as read:", error);
        // Revert optimistic update on failure if necessary, or just refresh
  ...
  const handleRoomNotification = useCallback(
    (notification) => {
      // ✅ CHANGE 1: Extract sender from notification
      const { type, roomId, lastMessage, sender } = notification;
      if (type === "new_message") {
        setRooms((prevRooms) => {
          const roomIndex = prevRooms.findIndex((r) => r.roomId === roomId);

          if (roomIndex === -1) {
            return prevRooms; // Room not in list
          }

          const currentRoom = prevRooms[roomIndex];

          // Check conditions step by step
          const notInCurrentRoom = currentRoomDetails?.roomId !== roomId;
          const notSender = sender?._id !== authUser?._id;
          const shouldIncrement = notInCurrentRoom && notSender;

          const newUnreadCount = shouldIncrement
            ? (currentRoom.unreadCount || 0) + 1
            : 0;
        ...
        // For user_joined, user_left, etc., a full refresh is safer
        refreshRoomList(true);
      }
    },
    // ✅ CHANGE 3: Add authUser to dependency array
    [currentRoomDetails, refreshRoomList, markRoomAsRead, authUser]
  );

  useEffect(() => {
    if (socket) {
      addEventListener("room_notification", handleRoomNotification);
      return () => {
        removeEventListener("room_notification", handleRoomNotification);
      };
    }
  }, [socket, addEventListener, removeEventListener, handleRoomNotification]);
  ...
  useEffect(() => {
    ...
    if (isRoomRoute) {
      ...
      const cachedDetails = storage.get(
        `${STORAGE_KEYS.ROOM_DETAILS}_${roomId}`
      );
      if (cachedDetails) {
