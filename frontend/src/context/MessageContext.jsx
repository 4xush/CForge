...
const setCachedMessages = (roomId, messages, hasMore = true) => {
  try {
    ...
    const limitedMessages =
      messages.length > CACHE_CONFIG.MAX_MESSAGES
        ? messages.slice(-CACHE_CONFIG.MAX_MESSAGES)
        : messages;
...
export const MessageProvider = ({ children }) => {
    ...
    async (lastMessageId = null) => {
      ...
      try {
        ...
        const query = lastMessageId
          ? `?lastMessageId=${lastMessageId}&limit=${limit}`
          : `?limit=${limit}`;
        ...
        if (
          ...
          currentRoomIdRef.current === currentRoomDetails._id
        ) {
          setLastSeenMessageId(response.data.lastSeenMessageId);
  ...
  const editMessage = useCallback(async (messageId, newContent) => {
    ...
    try {
      const response = await api.put(`/rooms/messages/${messageId}`, {
        content: newContent,
      });
